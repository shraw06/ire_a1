...
- **Evaluator at Large Scale**: pointing `run_eval.py` at `DATA_SCALE=large` behavior tables (outside the assignment's required scope — see §11) surfaced the same eager-materialization pattern and crashed the local machine loading entire behavior and score tables simultaneously.
...
The full evaluator (AUC, MRR, nDCG@5/10, ILD, Novelty, Coverage, slicing, bootstrap CIs) was implemented and run to completion at the small/demo scale (`results/eval_summary.csv`), which is the scale the assignment specifies for pipeline development and offline evaluation. The large bundles are called for specifically and only to generate Codabench test predictions, not to re-run offline evaluation - the large test sets carry no ground-truth labels, so AUC/MRR/nDCG/ILD/Novelty/Coverage cannot be computed against them in the first place; only recall@K against retrieval candidates is meaningful there, which is why §6 reports recall at large scale separately.

An early attempt to point the same evaluator at `DATA_SCALE=large` behavior tables (for our own curiosity, not because it was required) surfaced the same eager-materialization memory pattern noted in §3 - the evaluator loads full behavior and score tables at once, which does not fit comfortably in memory at EB-NeRD-large's ~24.6M rows. Since this scale of offline evaluation is outside the assignment's requirements, no further engineering time was spent making the evaluator memory-safe at that scale; the small/demo-scale run stands as the complete, required offline evaluation. The reranker tuning scripts in §6.1 are a distinct case - those *do* run at large scale because tuning depends on the same cached retrieval scores used for the (required) large-scale Codabench submissions, and that scaling issue was fixed (single-pass JSON parsing, vectorized AUC, per-dataset checkpointing) since it directly blocked a required deliverable.
